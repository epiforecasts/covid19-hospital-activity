...
scatter_data %>%
  ...
  scale_color_fermenter(breaks = integer_breaks(), palette = "RdBu", limits = max(abs(scatter_data$change))*c(-1, 1)) +
  # scale_color_distiller(palette = "RdBu", limits = max(abs(scatter_data$change))*c(-1, 1)) +
...
file_names <- list.files(file_dir)[which(!grepl("latest", list.files(file_dir)) &
                                           !grepl("samples", list.files(file_dir)))]
