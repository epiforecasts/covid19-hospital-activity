...
forecast_date <- as.Date("2021-08-22")
...
plot_forecast_bars(observed = region_obs,
  ...
  scale_x_date(limits = c(forecast_date - 63, forecast_date + 1),
               breaks = "2 weeks", date_labels = "%d %b") +
  scale_y_continuous(limits = c(0, 2500), breaks = integer_breaks()) +
    labs(title = "National",
       subtitle = "Past 14-day ahead forecasts, ensemble model") +
  theme(legend.position = "top")

plot_forecast_bars(observed = region_obs,
                   forecast = all_reg_forecasts %>% dplyr::filter(!(horizon == 14 & id == "London" & model == "mean_ensemble" & forecast_from == as.Date("2021-07-04"))),
                   h = 14,
                   use_dates = unique(all_reg_forecasts$forecast_from)[which(unique(all_reg_forecasts$forecast_from) < forecast_date)],
                   trusts = setdiff(unique(region_forecast$id), "England"),
                   trust_facet_var = "id",
