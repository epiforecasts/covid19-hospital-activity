...
return_region_plots <- function(region = "North West"){
  ...
  g_scatter <- scatter_data %>%
    ...
    scale_y_continuous(limits = c(0, NA), breaks = seq(0, 40, 4)) +
...
plot_forecast_ribbons(observed = admissions %>%
                        ...
                        dplyr::rename(observed = adm),
                      forecast = current_forecast,
                      trusts = focus_trusts$id,
                      models = c("ts_ensemble", "regression_arima", "convolution_rt"),
                      facet_models = TRUE) +
  scale_x_date(breaks = "1 weeks", date_labels = "%d %b") +
  scale_y_continuous(breaks = integer_breaks(), limits = c(0, NA)) +
  labs(title = "Top 5 Trusts with biggest probability of increase",
       subtitle = "Time series ensemble, ARIMA regression, and convolution",
       caption = paste0("Forecasts from ", format.Date(forecast_date, format = "%d %B %Y"))) +
  theme(legend.position = "none", strip.text.x = element_text(size = 8)) +
  ## Adding current observed data (dashed)
  geom_line(data = admissions %>%
...
plot_forecast_bars(observed = region_obs,
                   forecast = all_reg_forecasts %>% dplyr::filter(model == "mean_ensemble"),
                   h = 14,
                   use_dates = unique(all_reg_forecasts$forecast_from)[which(unique(all_reg_forecasts$forecast_from) < forecast_date)],
                   trusts = "England",
                   trust_facet_var = "id",
                   models = c("mean_ensemble"),
                   facet_models = FALSE) +
  scale_x_date(limits = c(forecast_date - 56, forecast_date + 1),
               breaks = "2 weeks", date_labels = "%d %b") +
  scale_y_continuous(limits = c(0, NA), breaks = integer_breaks()) +
    labs(title = "National",
       subtitle = "Past 14-day ahead forecasts, ensemble model") +
...
plot_forecast_bars(observed = region_obs,
                   forecast = all_reg_forecasts %>% dplyr::filter(model == "mean_ensemble") %>% dplyr::filter(!(horizon == 14 & id == "London" & model == "mean_ensemble" & forecast_from == as.Date("2021-07-04"))),
                   h = 14,
                   use_dates = unique(all_reg_forecasts$forecast_from)[which(unique(all_reg_forecasts$forecast_from) < forecast_date)],
                   trusts = setdiff(unique(region_forecast$id), "England"),
                   trust_facet_var = "id",
  ...
  scale_x_date(limits = c(forecast_date - 56, forecast_date + 1),
               breaks = "2 weeks", date_labels = "%d %b") +
  scale_y_continuous(limits = c(0, 300), breaks = integer_breaks()) +
    labs(title = "NHS regions",
       subtitle = "Past 14-day ahead forecasts, ensemble model") +
  theme(legend.position = "none")
