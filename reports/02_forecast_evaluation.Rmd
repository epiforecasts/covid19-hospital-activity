...
fill_models <- scale_fill_manual(breaks = main_model_labels, 
                                 values = c("lightgrey", "#66C2A5", "#FC8D62", "#8DA0CB", "#E78AC3"))
...
# By time (used in the WIS fig. S6)
...
g_rwis_horizon <- rank_horizon %>%
  ...
  ggplot(aes(y = label, x = scaled_rel_skill - 1, shape = horizon)) +
  geom_point(size = 4) +
  geom_vline(xintercept = 0, lty = 2) +
  scale_x_continuous(limits = c(-0.25, NA),
                     breaks = seq(-1, 2, 0.2),
                     labels = seq(-1, 2, 0.2) + 1) +
  scale_fill_distiller(palette = "RdBu", na.value = "grey80",
                       limits = c(-0.4, 0.4)) +
  labs(x = "Scaled WIS", y = "",
       title = "B",
...
(g_overall_calibration / (g_rwis_horizon) + plot_layout(heights = c(1.5, 1))) | g_individual_rank

ggsave(filename = here::here("data", "out", "figures", "figure_2.png"),
...
g_wiscontribution_horizon <- wis_horizon %>%
  ...
  dplyr::left_join(tb_main, by = "model") %>%
  dplyr::mutate(label = ordered(label, main_model_labels)) %>%
  ggplot(aes(y = label, x = value_norm, fill = name)) +
  geom_col() +
  facet_wrap(. ~ horizon) +
...
ggsave(filename = here::here("data", "out", "figures", "figure_s6.png"),
       width = 14, height = 8, units = "in",
       dpi = 500)
...
ggsave(filename = here::here("data", "out", "figures", "figure_3.png"),
       width = 14, height = 8, units = "in",
       dpi = 500)
...
ggsave(filename = here::here("data", "out", "figures", "figure_s7.png"),
       width = 14, height = 8, units = "in",
       dpi = 500)
...
g_location_mae0 <- g_location_mae0 +
  geom_point(data = mae_trust %>%
               filter(horizon == 7, rank == 1, id %in% top_admissions_x$id) %>%
               dplyr::left_join(tb_main, by = "model") %>%
               dplyr::mutate(label = ordered(label, main_model_labels)),
             aes(col = label), size = 3, alpha = 0.4) +
  geom_point(data = mae_trust %>%
               filter(horizon == 7, grepl("median_ensemble", model), id %in% top_admissions_x$id) %>%
               dplyr::left_join(tb_main, by = "model") %>%
               dplyr::mutate(label = ordered(label, main_model_labels)),
             aes(col = label), size = 3) +
  geom_point(data = mae_trust %>%
               filter(horizon == 7, model == "snaive", id %in% top_admissions_x$id),
...
ggsave(filename = here::here("data", "out", "figures", "figure_4.png"),
...
g_location_mae0 <- g_location_mae0 +
  geom_point(data = mae_trust %>%
               filter(horizon == 14, rank == 1, id %in% top_admissions_x$id, model != "snaive") %>%
               dplyr::left_join(tb_main, by = "model") %>%
               dplyr::mutate(label = ordered(label, main_model_labels)),
             aes(col = label), size = 3, alpha = 0.4) +
  geom_point(data = mae_trust %>%
               filter(horizon == 14, grepl("median_ensemble", model), id %in% top_admissions_x$id) %>%
               dplyr::left_join(tb_main, by = "model") %>%
               dplyr::mutate(label = ordered(label, main_model_labels)),
             aes(col = label), size = 3) +
  geom_point(data = mae_trust %>%
               filter(horizon == 14, model == "snaive", id %in% top_admissions_x$id),
             col = "grey50", shape = 4, stroke = 1, size = 2)
...
ggsave(filename = here::here("data", "out", "figures", "figure_s8.png"),
       width = 14, height = 10, units = "in",
       dpi = 500)
...
rank_obs_time <- pw_obs_time %>%
  ...
  dplyr::select(horizon, forecast_from, model, scaled_rel_skill, relative_skill, rank)
...
rank_obs_trust <- pw_obs_trust %>%
  ...
  dplyr::mutate(rank = rank(scaled_rel_skill, ties.method = "min")) %>%
  dplyr::arrange(horizon, id, rank) %>%
  dplyr::select(horizon, id, model, scaled_rel_skill, relative_skill, rank)
...
g_value_time7 <- rank_obs_time %>%
  ...
  scale_x_date(breaks = plot_dates[seq(2, 18, 4)], date_labels = "%d %b %y",
               limits = c(as.Date("2020-10-01"), as.Date("2021-02-03"))) +
  scale_y_continuous(limits = c(0.6, 1.7), breaks = seq(0, 2, 0.2)) +
  color_models +
  labs(x = "Forecast date", y = "Relative WIS",
       title = "D",
       col = "Model") +
...
guide_area() /
  ((g_value_calibration | (g_value_horizon + plot_layout(guides = "keep"))) |
     g_value_rwisrank + plot_layout(guides = "keep")) /
  (g_value_time7 | g_value_time14) +
  (g_value_trust7 | g_value_trust14) +
  plot_layout(heights = c(0.2, 1, 1, 1.5), guides = "collect")

ggsave(filename = here::here("data", "out", "figures", "figure_s10.png"),
...
ggsave(filename = here::here("data", "out", "figures", "figure_s11.png"),
       width = 14, height = 8, units = "in",
       dpi = 500)
...
g_outl_scatter <- rank_time %>%
  ...
  dplyr::mutate(model = ordered(model, main_models),
                label = ifelse(scaled_rel_skill.y < 0.9*scaled_rel_skill.x,
                               ...
                               NA)) %>%
  dplyr::left_join(tb_main, by = "model") %>%
  dplyr::mutate(label.y = ordered(label.y, main_model_labels)) %>%
  ggplot(aes(x = scaled_rel_skill.x, y = scaled_rel_skill.y, col = label.y)) +
  geom_point(size = 2) +
  geom_text_repel(aes(label = label.x)) +
...
ggsave(filename = here::here("data", "out", "figures", "figure_s12.png"),
       width = 14, height = 8, units = "in",
       dpi = 500)
