...
g_swis_horizon <- rank_horizon %>%
  ...
  ggplot(aes(y = label, x = swis - 1, col = horizon)) +
  geom_point(size = 4) +
...
g_fig2 <- (g_overall_calibration | g_swis_horizon) /
  g_individual_rank
...
g_wiscontrib <- tb_scores %>%
  dplyr::mutate(p_Sharpness = sharpness/interval_score,
                p_Underprediction = underprediction/interval_score,
                p_Overprediction = overprediction/interval_score) %>%
  dplyr::select(horizon, model, label, contains("p_")) %>%
  tidyr::pivot_longer(cols = contains("p_")) %>%
  dplyr::mutate(name = str_remove(name, "p_"),
                label = ordered(label, rev(main_model_labels))) %>%
  ggplot(aes(y = label, x = value, fill = name)) +
  geom_col() +
  geom_vline(xintercept = 0.333, col = "white", lty = 2) +
  geom_vline(xintercept = 0.666, col = "white", lty = 2) +
  facet_wrap(horizon ~ .) +
  scale_x_continuous(breaks = seq(0, 1, 0.2)) +
...
g_figure_s7 <- (g_location_scatter | g_location_scattermae) /
  (g_location_rank + plot_layout(guides = "keep")) +
  plot_layout(heights = c(3, 1.5), guides = "collect")

ggsave(plot = g_figure_s7,
...
compare_horizon <- rank_horizon %>%
  dplyr::select(horizon, model_for = model, swis_for = swis) %>%
  dplyr::left_join(tb_main,
                   by = c("model_for" = "model")) %>%
  dplyr::left_join(tb_obs,
                   by = "label")  %>%
  dplyr::left_join(rank_obs_horizon %>%
                     dplyr::select(horizon, model, swis_obs = swis),
                   by = c("horizon", "model")) %>%
  dplyr::select(horizon, label, swis_for, swis_obs) %>%
  dplyr::mutate(label = ordered(label, main_model_labels),
                swis_for = round(swis_for, 2),
                swis_obs = round(swis_obs, 2))
...
g_fig_s9 <- (g_value_arima_time | g_value_arima_trust) /
...
g_fig_s10 <- ((g_value_calibration | (g_value_horizon + plot_layout(guides = "keep"))) /
     g_value_rwisrank + plot_layout(guides = "keep"))

ggsave(plot = g_fig_s10,
       filename = here::here("data", "out", "figures", "figure_s10.png"),
       width = 14, height = 8, units = "in",
       dpi = 500)

g_fig_s11 <- guide_area() /
  (g_value_time7 | g_value_time14) /
  (g_value_trust7 | g_value_trust14) +
  plot_layout(heights = c(0.2, 1, 1), guides = "collect")

ggsave(plot = g_fig_s11,
       filename = here::here("data", "out", "figures", "figure_s11.png"),
       width = 14, height = 8, units = "in",
       dpi = 500)
