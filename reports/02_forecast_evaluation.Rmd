---
title: "Score admissions forecasts"
...
# Load forecasts (quantile) - for empirical coverage
...
main_model_names <- c("Baseline",
                      "Time series\nensemble",
                      "ARIMA\nregression",
                      "Convolution",
                      "Median\nensemble")

tb_main <- tibble::tibble(model = main_models, label = main_model_names)
...
g_overall_calibration <- emp_coverage_dat %>%
  ...
  dplyr::mutate(label = ordered(label, main_model_names)) %>%
...
g_individual_rank <- rank_all %>%
  ...
  dplyr::mutate(p = n/sum(n),
                ...
                label = ordered(label, main_model_names)) %>%
...
adm_scores %>%
  dplyr::filter(model %in% main_models) %>%
  dplyr::group_by(model) %>%
  dplyr::summarise(mean_ae = round(mean(aem, na.rm = TRUE), 2)) %>%
...
g_wiscontribution_horizon <- wis_horizon %>%
  dplyr::filter(horizon != 1,
                ...
                model != "snaive") %>%
  dplyr::mutate(model = ordered(model, levels = main_models)) %>%
  ...
  ggplot(aes(y = model, x = value, fill = name)) +
  geom_col() +
  facet_wrap(. ~ horizon) +
...
# Scaled relative skill ranking
rank_time <- pw_time %>%
  dplyr::select(forecast_from, horizon, model, scaled_rel_skill) %>%
  unique() %>%
...
### By time, 7-day horizon (fig. 3)
```{r by time plot 7 days}
...
g_time_obs <- observed %>%
  ...
  dplyr::mutate(median_adm = median(all_adm, na.rm = TRUE),
                   mean_adm = mean(all_adm, na.rm = TRUE)) %>%
  ggplot() +
...
g_time_rank <- rank_time %>%
  ...
  ggplot(aes(x = forecast_from, y = rank, col = model)) +
  geom_bump(lwd = 0.6) +
  geom_point(size = 3) +
...
# Relative skill value ts
g_time_rwis0 <- rank_time %>%
  dplyr::filter(horizon == 14) %>%
  dplyr::filter(model != "snaive") %>%
  ggplot(aes(x = forecast_from, y = scaled_rel_skill, col = model)) +
  geom_hline(yintercept = 1, lty = 2, lwd = 0.6, col = "grey50") +
  geom_point(size = 2, alpha = 0) +
  geom_point(data = rank_time %>%
               dplyr::filter(horizon == 14) %>%
               dplyr::filter(model != "snaive",
                             model != "convolution_rt"),
             size = 2) +
  geom_bump(data = rank_time %>%
               dplyr::filter(horizon == 14) %>%
               dplyr::filter(model != "snaive",
                             model != "convolution_rt"),
            lwd = 0.6) +
...
guide_area() / g_time_rwis1 / g_time_rwis0 / g_time_mae / plot_layout(guides = "collect", heights = c(0.2, 1, 1, 1))
...
rank_time %>%
  dplyr::filter(horizon == 7) %>%
  dplyr::group_by(model) %>%
  dplyr::summarise(med_rwis = median(scaled_rel_skill),
                   iq1_rwis = quantile(scaled_rel_skill, 0.25),
                   iq3_rwis = quantile(scaled_rel_skill, 0.75)) %>%
...
rank_trust <- pw_trust %>%
  ...
  dplyr::left_join(plot_50trusts, by = "id") %>%
  dplyr::ungroup()
...
```{r}
# 
# # Pairwise comparison, excluding ensemble model
# pw_trust_ind <- scoringutils::pairwise_comparison(adm_scores %>%
#                                                 dplyr::filter(model %in% setdiff(main_models, "mean_ensemble_observed")),
#                                               baseline = "snaive",
#                                               by = c("horizon", "model", "forecast_from", "id"),
#                                               summarise_by = c("id", "model"))
# 
# # Ranking
# rank_trust_ind <- pw_trust_ind %>%
#   dplyr::select(id, model, scaled_rel_skill) %>%
#   unique() %>%
#   dplyr::group_by(id) %>%
#   dplyr::mutate(rank = rank(scaled_rel_skill, ties.method = "min")) %>%
...
### By trust, 7-day horizon (fig. 4)
```{r}

# Relative WIS (tile)
g_location_tile <- rank_trust %>%
  dplyr::filter(id %in% plot_50trusts$id,
                horizon == 7,
                model != "snaive") %>%
...
g_location_rwis <- ggplot() +
  ...
  geom_rect(data = rwis_lines, aes(xmin = q1, xmax = q3, ymin = 0, ymax = Inf, fill = model), alpha = 0.2) +
  geom_histogram(data = rwis_hist, aes(x = scaled_rel_skill), binwidth = 0.02) +
  geom_vline(data = rwis_lines, aes(col = model, xintercept = q2), lwd = 0.8) +
  scale_color_brewer(palette = "Set2") +
...
g_location_rank <- rank_trust %>%
  ...
  dplyr::mutate(p = n/sum(n),
                rank = as.character(rank),
                model = ordered(model, main_models)) %>%
...
g_location_mae0 <- mae_trust %>%
  ...
  geom_point(aes(col = model), shape = 1, stroke = 1, size = 2, alpha = 0) +
...
g_location_mae0 <- g_location_mae0 +
  geom_point(data = mae_trust %>% filter(horizon == 7, rank == 1, id %in% plot_50trusts$id),
             aes(col = model), size = 3, alpha = 0.4) +
  geom_point(data = mae_trust %>% filter(horizon == 7, grepl("median_ensemble", model), id %in% plot_50trusts$id),
             aes(col = model), size = 3) +
  geom_point(data = mae_trust %>% filter(horizon == 7, model == "snaive", id %in% plot_50trusts$id),
             col = "grey50", shape = 4, stroke = 1, size = 2)


#####

guide_area() / (g_location_rwis + g_location_mae0) / (g_location_rank + plot_layout(guides = "keep")) +
...
### By trust, 14-day horizon (fig. Sx)
```{r}
...
g_location_rwis <- ggplot() +
  geom_vline(xintercept = 1, lty = 2, lwd = 0.8, col = "lightgrey") +
  geom_rect(data = rwis_lines, aes(xmin = q1, xmax = q3, ymin = 0, ymax = Inf, fill = model), alpha = 0.2) +
  geom_histogram(data = rwis_hist, aes(x = scaled_rel_skill), binwidth = 0.04) +
  geom_vline(data = rwis_lines, aes(col = model, xintercept = q2), lwd = 0.8) +
...
g_location_mae0 <- g_location_mae0 +
  geom_point(data = mae_trust %>% filter(horizon == 14, rank == 1, model != "snaive", id %in% plot_50trusts$id),
             aes(col = model), size = 3, alpha = 0.4) +
  geom_point(data = mae_trust %>% filter(horizon == 14, grepl("median_ensemble", model), id %in% plot_50trusts$id),
             aes(col = model), size = 3) +
  geom_point(data = mae_trust %>% filter(horizon == 14, model == "snaive", id %in% plot_50trusts$id),
             col = "grey50", shape = 4, stroke = 1, size = 2)
...
### rWIS tile, top 50 Trusts (fig. Sx)
```{r}
...
Also see "Overall - ARIMA regression" section.
...
ensemble_models <- c("snaive", "median_ensemble_observed", "median_ensemble_forecast")

# By horizon
pw_ensemble_horizon <- scoringutils::pairwise_comparison(adm_scores %>%
                                                  dplyr::filter(model %in% ensemble_models),
                                                baseline = "snaive",
                                                by = c("horizon", "model", "forecast_from", "id"),
                                                summarise_by = c("horizon", "model"))
rank_ensemble_horizon <- pw_ensemble_horizon %>%
  dplyr::select(horizon, model, scaled_rel_skill) %>%
  unique() %>%
  dplyr::group_by(horizon) %>%
  dplyr::mutate(rank = rank(scaled_rel_skill, ties.method = "min")) %>%
...
# By Trust
pw_ensemble_trust <- scoringutils::pairwise_comparison(adm_scores %>%
                                                  dplyr::filter(model %in% ensemble_models),
                                                baseline = "snaive",
                                                by = c("horizon", "model", "forecast_from", "id"),
                                                summarise_by = c("horizon", "id", "model"))

rank_ensemble_trust <- pw_ensemble_trust %>%
  dplyr::select(horizon, id, model, scaled_rel_skill) %>%
  unique() %>%
  dplyr::group_by(horizon, id) %>%
  dplyr::mutate(rank = rank(scaled_rel_skill, ties.method = "min")) %>%
...
rank_ensemble_trust %>%
  ...
  ggplot(aes(x = median_ensemble_forecast, y = median_ensemble_observed)) +
  ...
  labs(x = "rWIS ensemble model, with forecast cases", y = "rWIS ensemble model, with observed cases") +
  theme_bw()

```


### Value of perfect information (fig. Sx)
```{r}
...
g_value_trust7 <- ggplot() +
  ...
  scale_x_continuous(limits = c(0.35, 1.7), breaks = seq(0.2, 2, 0.2)) +
  scale_color_brewer(palette = "Set2") +
  scale_fill_brewer(palette = "Set2") +
  facet_grid(rows = vars(model)) +
  labs(x = "Relative WIS", y = "Number of Trusts",
...
g_value_arima_trust <- rank_arima_trust %>%
  ...
  scale_x_continuous(limits = c(0, NA)) +
  scale_y_continuous(limits = c(0, NA)) +
  labs(x = "rWIS ARIMA regression, forecast cases",
       y = "rWIS ARIMA regression, observed cases",
       title = "A") +
  theme_bw()
...
g_value_arima_time <- rank_arima_time %>%
  ...
  scale_x_date(breaks = plot_dates[seq(2, 18, 4)], date_labels = "%d %b %y",
               limits = c(as.Date("2020-10-01"), as.Date("2021-02-03"))) +
  scale_y_continuous(limits = c(0.5, NA), breaks = seq(0, 2, 0.2)) +
  scale_color_brewer(palette = "Set2") +
  labs(x = "Forecast date", y = "Relative WIS",
       title = "B",
       col = "Model") +
...
g_value_convolution_trust <- rank_convolution_trust %>%
  ...
  scale_x_continuous(limits = c(0, NA)) +
  scale_y_continuous(limits = c(0, NA)) +
  labs(x = "rWIS convolution, forecast cases",
       y = "rWIS convolution, observed cases",
       title = "C") +
  theme_bw()
